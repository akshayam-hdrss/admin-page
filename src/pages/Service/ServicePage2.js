...
export default function ServicePage() {
  ...
  const fetchServices = async () => {
    try {
      const res = await getServicesByType(serviceTypeId);
  ...
  const handleEdit = (service) => {
    setForm({
      ...
      order_no: service.order_no ?? "", // ✅ include order_no
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        order_no: form.order_no === "" ? null : Number(form.order_no), // ✅ ensure number
      };
            ...
            <div className="col-md-4 mb-4" key={item.id}>
              <div className="card h-100 shadow-sm">
                ...
                <div className="card-body" onClick={() => handleonclick(item.id)}>
                  ...
                  <h6>Order No: {item.order_no != null ? item.order_no : "null"}</h6>
              ...
              <div className="modal-content">
                <form onSubmit={handleSubmit}>
                  <div className="modal-header">
                    <h5 className="modal-title">{editingId ? "Edit" : "Add"} Service</h5>
                      ...
                      {[
                        ...
                        <div className="col-md-6" key={field}>
                          <input
                            ...
                            placeholder={field}
                            required={["serviceName"].includes(field)}
                          />
                        </div>
                      ))}
