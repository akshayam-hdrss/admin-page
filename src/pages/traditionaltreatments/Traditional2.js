  ...
  useEffect(() => {
    const fetchHospitals = async () => {
      try {
        ...
        let fetched = response.data.resultData;

        // ✅ sort by order_no (string, but numeric order if possible)
        fetched.sort((a, b) => {
          const aOrder = a.order_no ? Number(a.order_no) : Infinity;
          const bOrder = b.order_no ? Number(b.order_no) : Infinity;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...hospitalData,
        order_no:
          hospitalData.order_no !== "" && hospitalData.order_no !== null
            ? String(hospitalData.order_no) // ✅ always string
            : null,
      };

      if (editHospitalId) {
        await updateTraditional({ id: editHospitalId, ...payload });
      } else {
        await createTraditional(payload);
      }
      const updatedResponse = await getTraditionalByType(hospitalTypeId);
      let fetched = updatedResponse.data.resultData;
      fetched.sort((a, b) => {
      const aOrder = a.order_no ? Number(a.order_no) : Infinity;
      const bOrder = b.order_no ? Number(b.order_no) : Infinity;
      return aOrder - bOrder;
          ...
          {hospitals.length > 0 ? (
            hospitals.map((hospital) => (
              <div key={hospital.id} className="ha-card">
                ...
                <p className="ha-info"><strong>Order No:</strong> {hospital.order_no ?? "null"}</p> {/* ✅ show order_no */}
                ...
                <div className="ha-actions">
                  <Link
                    to={`/traditional/${hospitalTypeId}/${hospital.id}`}
          ...
          <div className="ha-modal-overlay">
            <div className="ha-modal">
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="ha-field">
                  <label>Order No</label> {/* ✅ new input */}
                  <input
                    type="text"
                    name="order_no"
                ...
                <div className="ha-field">
                  ...
                  <input
                    type="text"
                    name="hospitalTypeId"
                    value={hospitalData.traditionalTypeId}
                    readOnly
                  />
                </div>
